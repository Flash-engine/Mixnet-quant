...
import math
from quantize.quantize_modules import QConv2d, QLinear
...
def QConv3x3Bn(in_channels, out_channels, stride, non_linear='ReLU'):
    return nn.Sequential(
        QConv2d(in_channels, out_channels, 3, stride, 1, bias=False),
        nn.BatchNorm2d(out_channels),
        NON_LINEARITY[non_linear]
    )

def QConv1x1Bn(in_channels, out_channels, non_linear='ReLU'):
    return nn.Sequential(
        QConv2d(in_channels, out_channels, 1, 1, 0, bias=False),
        nn.BatchNorm2d(out_channels),
        NON_LINEARITY[non_linear]
    )
...
class SqueezeAndExcite(nn.Module):
    def __init__(self, channels, squeeze_channels, se_ratio):
        ...
        self.se_reduce = QConv2d(channels, squeeze_channels, 1, 1, 0, bias=True)
        self.non_linear1 = NON_LINEARITY['Swish']
        self.se_expand = QConv2d(squeeze_channels, channels, 1, 1, 0, bias=True)
...
class GroupedConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0):
        ...
        for i in range(self.num_groups):
            self.grouped_conv.append(QConv2d(#NOTE
...
class MDConv(nn.Module):
    def __init__(self, channels, kernel_size, stride):
        ...
        for i in range(self.num_groups):
            self.mixed_depthwise_conv.append(QConv2d(#NOTE
...
class MixNet(nn.Module):
    ...
    mixnet_m = [(24,  24,  [3],          [1],    [1],    1, 1, 'ReLU',  0.0),
                (24,  32,  [3, 5, 7],    [1, 1], [1, 1], 1, 6, 'ReLU',  0.0),
                (32,  32,  [3],          [1, 1], [1, 1], 1, 3, 'ReLU',  0.0),
                (32,  40,  [3, 5, 7, 9], [1],    [1],    1, 6, 'Swish', 0.5),
                ...
                (120, 120, [3, 5, 7, 9], [1, 1], [1, 1], 1, 3, 'Swish', 0.5),
                (120, 120, [3, 5, 7, 9], [1, 1], [1, 1], 1, 3, 'Swish', 0.5),
                (120, 180, [3, 5, 7, 9], [1],    [1],    2, 6, 'Swish', 0.5),
    ...
    def __init__(self, net_type='mixnet_m', input_size=32, num_classes=100, stem_channels=24, feature_size=512, depth_multiplier=0.83):
        ...
        # last several layers
        self.head_conv =QConv1x1Bn(config[-1][1], feature_size)

        self.avgpool = nn.AvgPool2d(input_size//4, stride=1)
